...
struct Graph {
  ...
  void bfs_parallel(vidType source, weight_type* distances) 
  {
    ...
    uint8_t alpha =  14, beeta  = 24;//tuning parameters
    uint32_t  visited_edges = 0, last_time_visited_edges = 0;
    uint64_t frontier_edges = rowptr[source+1] - rowptr[source], unvisited_edges =  0;
    ...
    {                 
      ...
      uint32_t local_frontier_edges = 0;
      ...
      if (!is_small_diameter) {
        ...
        while (curr_frontier_size != 0) {
          ...
          if (is_top_down) {
            ...
            for (uint32_t idx = 0; idx < curr_frontier_size; ++idx) {
              ...
              for (uint32_t i = rowptr[src]; i < rowptr[src + 1]; ++i) {
                ...
                if (distances[src] + 1 < distances[dst]) {
                  ...
                  if (__sync_bool_compare_and_swap(&distances[dst], std::numeric_limits<weight_type>::max(), distances[src] + 1)) {
                    local_frontier[tid][local_frontier_size++] = dst;
                    local_frontier_edges += rowptr[dst + 1] - rowptr[dst];
                  }
                }
              }
            }
            ...
            #pragma omp atomic
            frontier_edges += local_frontier_edges;
          }
          //bottom-up approach
          else {
          ...
          #pragma omp single
          {
            ...
            unvisited_edges  =  M - visited_edges;
            if(is_top_down){
              if(frontier_edges > unvisited_edges/alpha && next_frontier_size > curr_frontier_size ) is_top_down = false;//change from TOP-DOWN to BOTTOM-UP
              else  is_top_down = true; //change from BOTTOM-UP to TOP-DOWN
            }
            else{
              if(next_frontier_size >= curr_frontier_size || next_frontier_size > N/beeta) is_top_down = false;
