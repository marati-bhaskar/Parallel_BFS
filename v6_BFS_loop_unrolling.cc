    ...
    {                 
      ...
      if (!is_small_diameter) {
        ...
        while (curr_frontier_size != 0) {
          ...
          for (uint32_t idx = 0; idx < curr_frontier_size / 4 * 4; idx += 4) { // Correct relational condition
          vidType src1 = curr_frontier[idx];
          vidType src2 = curr_frontier[idx + 1];
          vidType src3 = curr_frontier[idx + 2];
          vidType src4 = curr_frontier[idx + 3];

          for (uint32_t i = rowptr[src1]; i < rowptr[src1 + 1]; ++i) {
            vidType dst = col[i];
            if (distances[src1] + 1 < distances[dst]) {
              distances[dst] = distances[src1] + 1;
              local_frontier[tid][local_frontier_size++] = dst;
            }
          }
          for (uint32_t i = rowptr[src2]; i < rowptr[src2 + 1]; ++i) {
            vidType dst = col[i];
            if (distances[src2] + 1 < distances[dst]) {
              distances[dst] = distances[src2] + 1;
              local_frontier[tid][local_frontier_size++] = dst;
            }
          }
          for (uint32_t i = rowptr[src3]; i < rowptr[src3 + 1]; ++i) {
            vidType dst = col[i];
            if (distances[src3] + 1 < distances[dst]) {
              distances[dst] = distances[src3] + 1;
              local_frontier[tid][local_frontier_size++] = dst;
            }
          }
          for (uint32_t i = rowptr[src4]; i < rowptr[src4 + 1]; ++i) {
            vidType dst = col[i];
            if (distances[src4] + 1 < distances[dst]) {
              distances[dst] = distances[src4] + 1;
              local_frontier[tid][local_frontier_size++] = dst;
            }
          }
        }
        ...
        #pragma omp for schedule(static) nowait
        for (uint32_t idx = (curr_frontier_size / 4) * 4; idx < curr_frontier_size; ++idx) {
