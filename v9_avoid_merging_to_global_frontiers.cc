...
struct Graph {
  ...
  void alloc_ds()
  {
    ...
    for (int i = 0; i < max_threads; ++i) {
      ...
      next_local_frontier[i] = (vidType*)aligned_alloc(64, N * sizeof(vidType));
    }
  }
  ...
  void bfs_parallel(vidType source, weight_type* distances) 
  {
    ...
    {                 
      ...
      if (!is_small_diameter) {
        int threshold = 48 * 10;
        int iterations = 0;
        // vidType* next_local_frontier[tid] = (vidType*)aligned_alloc(64, (N/4) * sizeof(vidType));
        uint32_t next_local_frontier_size = 0;
        ...
        while (curr_frontier_size != 0) {
          if(frontier_size < threshold){
            ...
            iterations = 0;
            uint32_t chunk_size = (frontier_size + total_threads - 1) / total_threads; // Ceiling division
            uint32_t start = tid * chunk_size;
            uint32_t end = std::min(start + chunk_size, frontier_size);

            for (uint32_t i = start; i < end; ++i) {
              local_frontier[tid][local_frontier_size++] = frontiers[l][i];
            }
            while(local_frontier_size != 0 && iterations < 20){
              #pragma omp for schedule(static) nowait
              for (uint32_t idx = 0; idx < local_frontier_size; ++idx) {
                vidType src = local_frontier[tid][idx];
                for (uint32_t i = rowptr[src]; i < rowptr[src + 1]; ++i) {
                  vidType dst = col[i];
                  if (distances[src] + 1 < distances[dst]) {
                    distances[dst] = distances[src] + 1;
                    next_local_frontier[tid][next_local_frontier_size++] = dst;
                  }
                }
              }
              local_frontier_size =  next_local_frontier_size;
              std::swap(local_frontier, next_local_frontier);
              iterations++;
            }
            if(local_frontier_size != 0) {
              frontier_starting_index = __atomic_fetch_add(&next_frontier_size, local_frontier_size, __ATOMIC_SEQ_CST);
              for (uint32_t i = 0; i < local_frontier_size; ++i) {
                frontiers[1 - l][frontier_starting_index + i] = local_frontier[tid][i];
              }
            }
            #pragma omp barrier
            l = 1 - l; // Toggle between 0 and 1
            #pragma omp single
            {
