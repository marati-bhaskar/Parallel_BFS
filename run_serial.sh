...
#SBATCH --cpus-per-task=1          # Maximum CPUs per task
...
# Output CSV files
runtime_csv="bfs_serial_v1_runtime.csv"
throughput_csv="bfs_serial_v1_throughput.csv"
...
version=1
  bfs_source="bfs_serial.cc"
  bfs_executable="bfs_serial.out"
  ...
  for graph in "${graph_files[@]}"; do
    ...
    for ((run=1; run<=N; run++)); do
      output=$(taskset -c 0 ./$bfs_executable $graph 0)
